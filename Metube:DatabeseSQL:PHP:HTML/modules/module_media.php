...
/* manage my media video playlist */
...
abstract class Mycategory
{
	...
	{
		global $db;
		$results = $db->select(tbl($this->cat_tbl),"*"," category_id='$category_id'");
		if($db->num_rows>0)
		{
			return $results[0];
		}else{
	...
	{
		global $db;
		$results = $db->select(tbl($this->cat_tbl),"category_name"," category_id='$category_'");
		if($db->num_rows>0)
		{
			return $results[0];
		}else{
	...
	function add_category($array)
	{
		...
		}elseif(empty($name))
		{
			...
			$category_id = $db->insert(tbl($this->cat_tbl),$flds,$values);
						
			$category_id = $db->insert_id();
			if($default=='yes' || !$this->get_default_category())
				$this->make_default_category($category_id);
	 ...
	 */
	function make_default_category($category_id)
	{
		global $db;
		if($this->category_exists($category_id))
		{
			$db->update(tbl($this->cat_tbl),array("isdefault"),array("no")," isdefault='yes' ");
			$db->update(tbl($this->cat_tbl),array("isdefault"),array("yes")," category_id='$category_id' ");
...
*/	
	...
	{
		global $db;
		$cat_details = $this->category_exists($category_id);
		if(!$cat_details)
			e(lang("cat_exist_error"));
		...
		elseif($cat_details['isdefault'] == 'yes')
			...
			$pcat = $this->has_parent($category_id,true);
			
			//Checking if category is both parent and child
			if($pcat && $this->is_parent($category_id))
			{
				$to = $pcat[0]['category_id'];
				$has_child = TRUE;
			}
			elseif($pcat && !$this->is_parent($category_id)) //Checking if category is only child
			{
				$to = $pcat[0]['category_id'];
				$has_child = TRUE;
			}
			elseif(!$pcat && $this->is_parent($category_id)) //Checking if category is only parent
			{
				$to = NULL;
				$has_child = NULL;
				$db->update(tbl($this->cat_tbl),array('parent_id'),array('0')," parent_id = $category_id");
			}
				
			//Moving all contents to parent OR default category									
			$this->change_category($category_id,$to,$has_child);
			
			//Removing Category
			$db->execute("DELETE FROM ".tbl($this->cat_tbl)." WHERE category_id='$category_id'");
	 ...
	 */
	function get_default_category_id()
	{
	...
	function update_category($array) {
		...
		$cid = mysql_clean($array['category_id']);
		if(!empty($this->use_sub_cats)) {
			$flds[] = "parent_id";
			$values[] = $pcat;	
		}
		...
		if($this->get_cat_by_name($name) && $cur_name !=$name) {
			...
			e(lang("add_cat_no_name_err"));
		} elseif ($pcat == $category_id){
			e(lang("You can not make category parent of itself"));
		} else {
			$db->update(tbl($this->cat_tbl),$flds,$values," category_id='$category_id' ");
			if($default=='yes')
				$this->make_default_category($category_id);
